...
AI Agent 작업 지표 (기대효과 정량화)
  ...
  코드 변경 완료율        75%  (완료 3 / 실패 1)
  외부 CLI 대화형 세션    2건
    └ 세션 성공률          100%  (성공 2 / 실패 0, 2/2건 태깅됨)
```

- **로컬 처리 비율** = 라우팅된 작업 중 로컬에서 처리된 비율(= 사내 코드를 외부로 보내지 않은 비율).
- **코드 변경 완료율** = `outcome`이 `completed`/`failed`인 작업(자동 라우팅으로 실제 코드 수정을
  시도한 건)만 대상으로 한 성공률. 설명 모드(`explain`)나 잡담(`chatter`)은 분모에서 제외됩니다.
- **외부 CLI 대화형 세션(`/model claude`·`codex`)** 은 사람이 직접 조작하므로 자동으로 성공/실패를
  판정할 수 없습니다. 세션을 종료하면 "방금 세션에서 작업이 잘 끝났나요?"를 한 번 묻고, 답한 만큼만
  세션 성공률에 집계됩니다(건너뛰면 집계에서 제외 — 강제 아님).
